...
if(filter_has_var(INPUT_GET, 'submit'))
{
	...
	$tor_name = filter_input(INPUT_GET, 'tname');
	...
	else if(strlen($infohash) == 40 && ctype_xdigit($infohash))
	{
		...
		if($tor_name != "")
		{
			$magnetlink = $magnetlink . "&dn=" . $tor_name;
		}
		if($trackers != "")
		{
	...
	//$magnetlink = "magnet:?xt=urn:btih:" .$infohash . "&dn=" . $tor_name . "&tr=" . $trackers;
}
?>
<!DOCTYPE html>
<html lang="en">
		...
		<div class="w3-card-4">
			...
			<form class="w3-container" name="magnet" method="get" action="#">
				<p>
					...
					<input class="w3-input" type="text" name="infohash" value='<?php echo $infohash ?>' placeholder="Hash of the Torrent Ex: DA39A3EE5E6B4B0D3255BFEF95601890AFD80709" />
				</p>
				<p>
					<label>Name</label>
					<input class="w3-input" type="text" name="tname" value='<?php echo $tor_name ?>' placeholder="Name of the Torrent Ex:Linux 5.7" />
				</p>
				<p>
					<label>Trackers</label>
					<textarea  class="w3-input" name="trackers" placeholder="One tracker per line Ex: udp://tracker.opentrackr.org:1337/announce"><?php echo $trackers ?></textarea>
				</p>
				<p>
					<label>Webseeds</label>
					<input class="w3-input" type="text" name="webseeds" value='<?php echo $webseeds ?>' placeholder="Web Seeds Ex: http://example.com/linux.iso" />
				</p>
				<p>
